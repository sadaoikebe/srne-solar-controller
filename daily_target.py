...
def fetch_tomorrow_weather_code():
    ...
    try:
        ...
        southern_data = [
            ...
            if any(area['area']['name'] == '南部' for area in item['timeSeries'][0]['areas'])
        ]
...
def calculate_required_current(battery_soc, target_soc, charging_hours):
    ...
    required_energy_wh = soc_diff * 270  # SOC 1% あたりの Wh
...
def estimate_soc_at_2259(current_soc):
    ...
    energy_consumed_wh = 1000 * hours_until_2259   # 1 kW average load
    soc_decrease       = energy_consumed_wh / 255   # SOC 1% あたり255Wh
    estimated_soc      = max(0, current_soc - soc_decrease)
    print(f"Estimated SOC at 22:59: {estimated_soc:.2f} "
          f"(current SOC: {current_soc}, hours until 22:59: {hours_until_2259:.2f})")
...
def parse_args():
    ...
    parser.add_argument("--estimate-start-soc", action="store_true",
                        help="Estimate SOC at 22:59 based on 1kW average consumption")
    parser.add_argument("--start-soc", type=int,
                        help="Use this SOC value instead of fetching from registers")
    parser.add_argument("--target-soc", type=int,
                        help="Use this target SOC instead of weather-based calculation")
    parser.add_argument("--charging-hours", type=float,
                        help="Charging hours to use instead of calculating until a specific time")
    parser.add_argument("--weather-code", type=int,
                        help="Weather code to use instead of fetching from JMA")
    parser.add_argument("--dry-run", action="store_true",
                        help="Run in dry-run mode without writing to targets.json")
    parser.add_argument("--until-time",
                        help="Time to charge until in HH:MM format (e.g., 05:30). Default is 05:30 next morning.")
...
def main():
    ...
    if args.start_soc is not None:
        ...
        if not limited_data:
            ...
            return
        # Register key uses hex address to match modbus_api v2 schema:
        # 0x0100 = battery SOC (%)
