...
| `soc_estimator` | Pack SoC from JK `remain_ah` / current (+ inverter latch if BLE is down). `GET /soc` on port **5006**; writes **`soc_estimate`**. See [`docs/soc-control.md`](soc-control.md). |
